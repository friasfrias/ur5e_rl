...
class BalanceEnv(gym.Env):
    def __init__(self, render_window=True):
        ...
        self.observation_space = gym.spaces.Box(low=-180.0, high=180.0, shape=(2,), dtype=np.float32)
        self.action_space      = gym.spaces.Box(low=-1.0,  high=1.0,  shape=(2,), dtype=np.float32)

        # === Reward configuration ===
        self.survival_rate = 2
        self.dt = self.model.opt.timestep
        self.max_steps = 5000
        self.current_step = 0
        self.cumulative_reward = 0.0
        self.theta_ok = 5.0
        self.penalty_per_deg = 0.0001
        self.multiplier_inside = 10.0
        self.multiplier_outside = 1.0

        # Janela de silêncio pós-reset
        ...
        if self.render_window:
            ...
            for a in range(self.model.nu):
                # para actuators do tipo "joint", o primeiro índice aponta para o joint id
    ...
    # ====== Reward ======
    def _compute_reward(self, alpha, beta):
        penalty = self.penalty_per_deg * (abs(alpha) + abs(beta))
        live_bonus = self.multiplier_inside * self.dt if (abs(alpha) <= self.theta_ok and abs(beta) <= self.theta_ok) else self.multiplier_outside * self.dt
        return live_bonus - penalty

    # ====== Aplicação de trajectórias ======
    def cb_trajectory(self, msg):
        # Gate: ignorar durante reset
        ...
        # Aplicar por NOME → ATUADOR (ordem do ROS pode variar)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Notificar reset e abrir janela de silêncio
        self.reset_pub.publish(Bool(data=True))
        now_ns = self.node.get_clock().now().nanoseconds
        self._no_cmd_until_ns = now_ns + int(0.35 * 1e9)  # 350 ms silenciosos
        self._grace_steps = 8                              # 8 passos sem terminação por padrão
        self._just_reset = True                            # reabre gate no 1º step

        obs = np.asarray(self._last_angles, dtype=np.float32)
        return np.clip(obs, -180.0, 180.0).astype(np.float32), {}
    ...
    def step(self, action):
        ...
        # === /joint_states com nomes/posições/velocidades ALINHADOS ===
        stamp = self.node.get_clock().now().to_msg()
        js = JointState()
        ...
        # Reabrir gate no 1º step após reset
        if self._just_reset:
            self.ignore_joint_updates = False
            self._just_reset = False

        # === Detetar padrão (periodicamente) ===
        if self.current_step % 10 == 0:
            self.pattern_detected = detect_pattern(self.last_rgb, nx=5, ny=21)
        pattern_ok = self.pattern_detected

        # === Publicar ação (dx, dy) com janela de silêncio pós-reset ===
        now_ns = self.node.get_clock().now().nanoseconds
        ...
        self.current_step += 1
        # durante a janela de graça, não terminar por falta de padrão
        terminated = (not pattern_ok) and (not block_cmds)
        truncated  = (self.current_step >= self.max_steps)

        obs = np.clip(angles, -180.0, 180.0).astype(np.float32)
