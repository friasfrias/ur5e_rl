...
for Kp_v = 0.5:0.5:6
  for Ki_v = 0.01:0.01:0.1
    for dx_f = 5:5:30
      ...
      start_clock = tic;
      t_last_msg = 0;
      t_lost      = max_test;   % se nunca perder, fica nos 20s
      lost_flag   = false;
      ...
      while toc(start_clock) < max_test
          try
      ...
      if t_lost > best_time
        best_time = t_lost;
        best_cfg  = struct('Kp',Kp_v,'Ki',Ki_v,'dx_f',dx_f,'time',t_lost);
      end
